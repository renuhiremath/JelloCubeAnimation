...
void myinit()
{
  ...
  GLuint myTexture;
	Pic* img;
	{
  	...
  	pic_free(img);
  	jello.textureId[0] = myTexture;
	}
	{
		img = loadBitmap("face2.ppm");
		// allocate a texture name
		glGenTextures( 1, &myTexture );
		// select our current texture
		glBindTexture( GL_TEXTURE_2D, myTexture );
		...
		pic_free(img);
		jello.textureId[1] = myTexture;
	}
	{
  	img = loadBitmap("face3.ppm");
  	// allocate a texture name
  	glGenTextures( 1, &myTexture );
  	// select our current texture
  	glBindTexture( GL_TEXTURE_2D, myTexture );
  	...
  	pic_free(img);
  	jello.textureId[2] = myTexture;
	}
	{
  	img = loadBitmap("face4.ppm");
  	// allocate a texture name
  	glGenTextures( 1, &myTexture );
  	// select our current texture
  	glBindTexture( GL_TEXTURE_2D, myTexture );
  	...
  	pic_free(img);
  	jello.textureId[3] = myTexture;
	}
	{
  	img = loadBitmap("face5.ppm");
  	// allocate a texture name
  	glGenTextures( 1, &myTexture );
  	// select our current texture
  	glBindTexture( GL_TEXTURE_2D, myTexture );
  	...
  	pic_free(img);
  	jello.textureId[4] = myTexture;
	}
	{
  	img = loadBitmap("face6.ppm");
  	// allocate a texture name
  	glGenTextures( 1, &myTexture );
  	// select our current texture
  	glBindTexture( GL_TEXTURE_2D, myTexture );
...
void doIdle()
{
  char s[20]="pics/xxx.ppm";
  int i;

  // save screen to file
  //s[3] = 48 + (sprite / 1000);
  s[5] = 48 + (sprite % 1000) / 100;
  s[6] = 48 + (sprite % 100 ) / 10;
  s[7] = 48 + sprite % 10;

  if (saveScreenToFile == 15)
  {
